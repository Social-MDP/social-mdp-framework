...
@click.option('--chis', type=str, help="All possible Chi values", default="-2,-1,-0.5,0,0.5,1,2")
@click.option('--output', required=True, type=click.Path(), help="The output directory")
def make_plots(record, chis: str, output):
    chis = [float(chi.strip()) for chi in chis.split(',')]
    filename = os.path.split(record)[1]
    with open(record) as f:
        record = json.load(f)
    output = Path(output)
    output.mkdir(parents=True, exist_ok=True)
    visualize_estimations(record, chis).savefig(output / 'estimations.png')
